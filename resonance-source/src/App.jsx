...
import { ensureUser, getPair, getPartnerId, createPair, joinPair, canSendTrace, sendTrace, getPendingTrace, discoverTrace, getArtwork, dissolvePair, subscribeToTraces, subscribeToPair, subscribeToEvents } from './lib/supabase.js';
import { detectMoments } from './lib/moments.js';
import { TONES, TONE_KEYS, WHISPER_WORDS, ECHO_MARKS, GLIMPSE_TEXTS, FONT, lerp, dst, clamp, pick, hex2, makeNoise, analyzeGesture, drawGesturePath } from './lib/constants.js';
...
function PairSetup({ onPaired, userId }) {
  ...
  // Listen for partner joining when in waiting mode
  useEffect(function() {
    if (mode !== "waiting" || !invite) return;
    // Poll for pair status change (realtime subscription set up separately)
...
export default function App() {
    ...
    (async function() {
      try {
        ...
        try {
          ...
          // DB error (schema not loaded?) — still show welcome
          console.error("DB error:", dbErr);
          setAppPhase("welcome");
        }
      } catch (authErr) {
        console.error("Auth error:", authErr);
        setInitError("Connection failed: " + (authErr.message || String(authErr)) + ". Make sure Anonymous Auth is enabled in Supabase (Authentication → Providers → Anonymous Sign-Ins → ON) and the SQL schema has been run.");
      }
    })();
  }, []);
  ...
  if (appPhase === "pairing") {
    return <PairSetup onPaired={handlePaired} userId={user ? user.id : null} />;
  }

  // Space phase — the main experience
...
function ResonanceSpace({ user, pair, onDissolve }) {
  ...
  // Moment queue
  var _mq = useState([]), momentQueue = _mq[0], setMomentQueue = _mq[1];
  ...
  // ── Initial load: fetch artwork + pending trace ──
  useEffect(function() {
    if (!pair) return;
    (async function() {
      ...
      if (art.length > 0) {
        ...
        setOnbStep(4); // skip onboarding if there's existing artwork
      }
  ...
  // ── Realtime: listen for incoming traces ──
  useEffect(function() {
    if (!user) return;
    var sub = subscribeToTraces(user.id, function(newTrace) {
      // New trace arrived from partner!
      if (phR.current === "idle") {
        setTrace(newTrace);
        setPhase("discovery");
      }
  ...
  // ── Passive reveal timer ──
  useEffect(function() {
    if (phase === "discovery" && trace && trace.passive_reveal) {
      var delay = 60000 + Math.random() * 30000;
      var t = setTimeout(function() { if (phR.current === "discovery") setPhase("revealing"); }, delay);
  ...
  useEffect(function() {
    ...
    function frame() {
      ...
      var hp = hpR.current, ph = phR.current, tr = trR.current, tc = tcR.current, rt = rtR.current, re = reR.current, cbs = cbR.current;
      ...
      // Artwork underlay
      if (cbs.length > 0) { cbs.forEach(function(c2) { drawGesturePath(ctx, c2.path, c2.tone, w, h, 0.04 + Math.sin(t*0.2)*0.01, 4); }); }
  ...
  // ── Reveal done → detect moments → queue them ──
  var onRevealDone = useCallback(async function() {
    ...
    // Detect ALL moments — queue them all
    var moments = await detectMoments(pair.id, user.id, tr, tr.emotional_tone);
    if (moments.length > 0) {
      setMomentQueue(moments);
      // Start first moment
      var first = moments[0];
      setMTone(first.tone);
      setMPhase(first.type + "_intro");
    } else {
      setPhase("glimpse");
    }
    hpR.current = 0;
  ...
  // ── Process moment queue: when current moment finishes, play next or go to glimpse ──
  var advanceMomentQueue = useCallback(function() {
    setMomentQueue(function(prev) {
      var remaining = prev.slice(1);
      if (remaining.length > 0) {
        var next = remaining[0];
        setMTone(next.tone);
        setMPhase(next.type + "_intro");
      } else {
        setMPhase(null);
        setPhase("glimpse");
      }
  ...
  // Moment intro → picker transitions
  ...
  var onWhisperSelect = useCallback(function(w) { setMPhase("whisperShow"); setWhisper(w); }, []);
  var onWhisperTimeout = useCallback(function() { advanceMomentQueue(); }, [advanceMomentQueue]);
  var onWhisperDone = useCallback(function() { setWhisper(null); advanceMomentQueue(); }, [advanceMomentQueue]);
  var onEchoSelect = useCallback(function(m) { setMPhase("echoShow"); setEchoM(m); }, []);
  var onEchoTimeout = useCallback(function() { advanceMomentQueue(); }, [advanceMomentQueue]);
  var onEchoDone = useCallback(function() { setEchoM(null); advanceMomentQueue(); }, [advanceMomentQueue]);
  var onPulseCapture = useCallback(function(p2) { if (p2) setPendPulse(p2); advanceMomentQueue(); }, [advanceMomentQueue]);
  ...
  var onSendTrace = useCallback(async function(data) {
    ...
    try {
      ...
      // Trace sent. Update local contribs.
  ...
  return (
    <div style={{ width:"100%",height:"100vh",position:"relative",overflow:"hidden",background:"#0A0A12",userSelect:"none",WebkitUserSelect:"none" }}>
      ...
      {showSettings ? <div style={{ position:"absolute",inset:0,zIndex:48,display:"flex",alignItems:"flex-end",justifyContent:"center" }} onClick={function() { setShowSettings(false); }}>
        ...
        <div onClick={function(ev) { ev.stopPropagation(); }} style={{ position:"relative",width:"100%",maxWidth:400,background:"#111118",borderRadius:"20px 20px 0 0",padding:"28px 24px 40px",fontFamily:FONT }}>
          ...
          <div style={{ padding:"16px 0" }}>
            <div onClick={function() { if (confirm("Dissolve this connection?")) { setShowSettings(false); onDissolve(); } }} style={{ color:"rgba(196,30,58,0.5)",fontSize:11,fontWeight:200,letterSpacing:"0.1em",cursor:"pointer" }}>Dissolve Connection</div>
          </div>
        </div>
      </div> : null}
      ...
      {/* Reveal — TODO: extract to component, keeping inline for now */}
...
// ── Sub-components (stubs that reference the same logic as the prototype) ──
// These would be the same canvas-based components from the prototype.
// For brevity I'm using placeholder implementations — the full code from
// the artifact prototype should be copied here.
...
function WhisperPickerUI({ rgb, onSelect, onTimeout }) {
  ...
  return <div style={{ position:"absolute",inset:0,zIndex:45,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:"rgba(6,6,12,0.93)",fontFamily:FONT,animation:"fadeIn 0.8s ease" }}>
    ...
    <div style={{ color:"rgba("+rgb+",0.6)",fontSize:12,letterSpacing:"0.2em",fontWeight:300,marginBottom:44 }}>choose a whisper</div>
...
function WhisperDisplayUI({ word, rgb, onDone }) {
  var _a = useState(0), al = _a[0], sa = _a[1];
  useEffect(function() { var s = Date.now(); var iv = setInterval(function() { var pr = (Date.now()-s)/8000; if (pr >= 1) { clearInterval(iv); onDone(); } else sa(pr<0.15?pr/0.15:pr>0.7?1-(pr-0.7)/0.3:1); }, 30); return function() { clearInterval(iv); }; }, [onDone]);
  return <div style={{ position:"absolute",top:"38%",left:0,right:0,textAlign:"center",zIndex:38,pointerEvents:"none",fontFamily:FONT }}><span style={{ fontSize:30,fontWeight:200,letterSpacing:"0.35em",color:"rgba("+rgb+","+(al*0.7)+")",textShadow:"0 0 35px rgba("+rgb+","+(al*0.25)+")" }}>{word}</span></div>;
}
...
function EchoMarkPickerUI({ rgb, onSelect, onTimeout }) {
  ...
  return <div style={{ position:"absolute",inset:0,zIndex:45,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:"rgba(6,6,12,0.93)",fontFamily:FONT,animation:"fadeIn 0.8s ease" }}>
    ...
    <div style={{ color:"rgba("+rgb+",0.6)",fontSize:12,letterSpacing:"0.2em",fontWeight:300,marginBottom:44 }}>leave a mark</div>
...
function EchoMarkDisplayUI({ mark, rgb, onDone }) {
  var _a = useState(0), al = _a[0], sa = _a[1];
  useEffect(function() { var s = Date.now(); var iv = setInterval(function() { var pr = (Date.now()-s)/10000; if (pr >= 1) { clearInterval(iv); onDone(); } else sa(pr<0.1?pr/0.1:1-(pr-0.1)/0.9); }, 100); return function() { clearInterval(iv); }; }, [onDone]);
  return <div style={{ position:"absolute",top:"28%",left:"50%",transform:"translate(-50%,-50%)",zIndex:9,pointerEvents:"none",fontFamily:FONT }}><span style={{ fontSize:42,color:"rgba("+rgb+","+(al*0.35)+")",textShadow:"0 0 25px rgba("+rgb+","+(al*0.12)+")" }}>{mark.g}</span></div>;
}
...
// Reveal canvas (simplified — uses same drawing logic)
