...
// "Publishable key" = the new name for what was "anon key"
// It's safe to use in the browser with RLS enabled.
// ═══════════════════════════════════════════
...
export async function createPair() {
  ...
  if (error) throw error;
  return data; // { pair_id, invite_code }
}
...
export async function joinPair(code) {
  ...
  if (error) throw error;
  return data; // { pair_id, status } or { error }
}
...
// ── Realtime: subscribe to resonance events ──
